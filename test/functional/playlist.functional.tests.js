...
describe('playlist', () => {
    ...
    const userWithPlaylist = new User('userWithPlaylist', [{ id: lastExistingPlaylistId, name: 'existingPlaylist' }]);
    ...
    it('should be created for a user having already a playlist', async () => {
        ...
        const savedPlaylist = savedUser.playlists.find(pl => pl.id == playlist.id)

        assert.equal(savedUser.playlists.length, userWithPlaylist.playlists.length + 1);
