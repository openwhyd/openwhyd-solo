...
const { mongodb, initMongoDb, cleanup, readMongoDocuments, insertUser } = require('./mongo.integration.base.tests');
const { userCollection: userRepository } = require('../../../app/infrastructure/mongodb/UserCollection');
...
describe('Mongo user repository should', function () {
  ...
  it('find user by id', async() => {
    ...
    assert.deepEqual(playlists, user[0].pl.map(pl => ({'id': Number(pl.id), 'name': pl.name})));
  });
  ...
  it('insert playlist to user', async() => {
    ...
    const userDocument = await mongodb.collections['user'].findOne({ _id: mongodb.ObjectId(userId) });
    assert.deepEqual([ newPlaylist ], userDocument.pl.filter(pl => pl.id == playlistId));
  });
});